# 💡 Project Overview: Uncovering Personality Trait Dimensions using Machine Learning
...
|---|---|---|
| `Data4036018.csv` | 10,000 simulated cases: binary response Y, relevant predictors X₁ – X₃, and 200 uniform noise variables X₄ - X₂₀₃. Generated with a personal seed via [GenerateDataSDS.R](GenerateDataSDS.R) | simulated |
...
**Simulation design (Part A).** The predictors are drawn as X₁ ~ U(-2, 2), X₂ ~ N(4, 2), X₃ ~ N(0, 1); all noise variables are U(-2, 2). The response is generated from a deliberately **non-linear** logit:
...
|---|---|---|
| Low-dimensional (X₁- X₆, k = 79) | **71.74%** | 67.28% |
| High-dimensional (X₁ - X₂₀₃, k = 41) | 57.14% | **67.02%** |
...
3. In the low-dimensional fit, LLR zeroed out X₅ and X₆ and shrank X₄ to ≈ 0.008, confirming its variable-selection behaviour even when it loses on accuracy
...
|---|---|---|---|
| Low-dim (X₁ - X₆) | kNN | k = 79 | **71.74%** |
| Low-dim (X₁ - X₆) | LLR | λ = 0.0103 | 67.28% |
| High-dim (X₁ - X₂₀₃) | kNN | k = 41 | 57.14% |
| High-dim (X₁ - X₂₀₃) | LLR | λ = 0.0059 | **67.02%** |
...
- The low-dimensional lasso fit shows exactly the behaviour it is known for: X₅ and X₆ shrunk to precisely zero, X₄ to ≈ 0.008, leaving essentially
...
- Kaiser's rule agrees (eigenvalues of PC1–PC5 are > 1; PC6 just misses with λ₆ = 0.995), and in parallel analysis the 6th eigenvalue falls exactly on the simulated mean
